...
- **文档预处理**：
  - 通过 `add_corpus` 方法，加载并处理多个 PDF 或其他格式的文档。每个文档被提取文本并使用 `SentenceSplitter` 类分割成小块，这些小块更适合进行快速的文本检索。
  
- **构建语料库**：
  - 分割后的文本块被添加到相似度模型的语料库中。这样，每个文本块都可以独立地作为信息检索的单元。

- **执行查询检索**：
  - 使用 `get_reference_results` 方法对用户的查询进行处理。该方法调用相似度模型（如集成了 BertSimilarity 和 BM25Similarity 的 EnsembleSimilarity），对语料库中的所有文本块进行相似度打分，选择与查询最相关的顶部文本块。

### 2. **内容增强 (Augmentation) 阶段**

- **二次排序 (Re-ranking)**：
  - 如果配置了重排序模型（例如 BERT 序列分类模型），该步骤对初步检索的结果进行进一步的排序，确保上下文的相关性和质量最高。

- **上下文构建**：
  - 选出的文本块（根据相似度得分和/或重排序结果）被组合成一个连贯的上下文字符串。这个字符串会与用户的原始查询一起构成生成模型的输入。

### 3. **回答生成 (Generation) 阶段**

- **格式化输入**：
  - 利用 `PROMPT_TEMPLATE`，结合检索到的上下文和用户的查询问题，格式化生成模型的输入。这种格式化的输入有助于模型理解问题的背景和具体的询问点。
  
- **调用生成模型**：
  - 将格式化的输入送入一个预训练的生成模型（如 GPT、BLOOM、LLaMA 等）。模型配置了生成参数（如温度、重复惩罚等），确保生成的回答既创新又准确。

- **生成文本**：
  - 模型输出的文本是对用户查询的直接回答，基于提供的上下文和内嵌的知识。

### 4. **输出和后续处理**

- **结果呈现**：
  - 生成的回答返回给用户，如果系统配置了历史记录功能，可以进一步保存对话历史用于分析或改进。
